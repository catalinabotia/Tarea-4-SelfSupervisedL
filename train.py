...
from core.optimization import optimization, downstream
...
def main(args):
    ...
    for epoch in range(init_epoch, epochs):
        ...
        csv = csv.append(pd.DataFrame({'epoch': [epoch + 1], 'loss': [loss], 'lr': [optimizer.param_groups[0]['lr']]}),
                         ignore_index=True)
        ...
        if ((epoch + 1) % config['eval_every_n_epochs']) == 0:
            # evaluate
            top_epoch, best_train_acc, best_acc = downstream(model, device,
